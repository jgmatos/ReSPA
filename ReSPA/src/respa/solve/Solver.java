...
public abstract class Solver {
	...
	public HashMap<String, InputVariable> solve() {
		...
		if(Core.currentPathCondition!=null)//if REAP outputs an alternative path
			return solve(Core.currentPathCondition);
		else if(Core.previousPathCondition!=null)//if not do it with the original path
			return solve(Core.previousPathCondition);
	...
	public HashMap<String, InputVariable> solve(PathCondition pc) {
		...
		if(pc!=null){
			...
			for(String sisname: stringsymbvar){
				...
				if(pc.spc.header!=null && 
						!Core.ignoreString && !Core.ignoreStringPC){
		...
		else if(Core.symbvars.values().size()>0){//No path condition

			symbvarsByName = Core.symbvars;

		}

		Core.delimiters=varsInPC.size();
	...
	private void fillWithSymbvars(ArrayList <String> stringsymbvar,ArrayList <String> intsymbvar,HashMap<String, InputVariable> symbvarsByName) {
		...
		ArrayList<InputVariable> symbvars = new ArrayList<InputVariable>(Core.symbvars.values());
