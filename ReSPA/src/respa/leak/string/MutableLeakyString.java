...
public class MutableLeakyString extends LeakyVariable{
	...
	public MutableLeakyString(int size,String id) {
	...
	private MutableLeakyString(ArrayList<LeakyChar> leakystring,
			ArrayList<Exclude> excluding,String concrete,int size,
			int domainSize,String id) {
	...
	public void applyNotEqualsCharAt(char c, int index) {
		...
		leakystring.get(index).applyNotEquals(c);
/*		if(excluding==null)
			excluding = new ArrayList<Exclude>();
	...
	public Object makeCopy() {
		...
		if(excluding ==null)
			return new MutableLeakyString(clonedlist, null, concrete, size, domainSize, id);
		else{

			ArrayList<Exclude> clonedExclude = new ArrayList<Exclude>();
			for(Exclude e: excluding)
				clonedExclude.add((Exclude)e.clone());
			return new MutableLeakyString(clonedlist, clonedExclude, concrete, size, domainSize, id);
		}
	...
	private void resize(int newsize) {
		...
		for(int i=size; i<newsize; i++)
			this.leakystring.add(new LeakyChar());
